// Generated on 2014-08-19 using generator-angular-fullstack 2.0.10
'use strict';
...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      ...
      loader: {
        files: {
          '<%= yeoman.dist %>/public/config/loader.js': ['<%= yeoman.client %>/config/loader.js']
        }
      },
      config: {
        files: {
          '.tmp/config/config.js': ['<%= yeoman.client %>/config/dev.*.js'],
          '<%= yeoman.dist %>/public/config/config.js': ['<%= yeoman.client %>/config/dev.config.js'],
        }
      },
      styles: {
        expand: true,
    ...
    nggettext_extract: {
      pot: {
        files: {
            'translations/<%= pkg.name %>/<%= pkg.name %>.pot': [
    ...
    'string-replace': {
      pot_project: {
        files: {
          'translations/<%= pkg.name %>/<%= pkg.name %>.pot': 'translations/<%= pkg.name %>/<%= pkg.name %>.pot',
        },
        options: {
          replacements: [{
  ...
  grunt.registerTask('translations-export', [
    'nggettext_extract',
    'string-replace:pot_project',
    'shell:pot2crowdin'
  ]);

  grunt.registerTask('translations-import', [
    'shell:crowdin2po',
    //'nggettext_compile'
  ]);
